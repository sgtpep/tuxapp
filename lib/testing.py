...
is_app_process_output_ignored = lambda app, distribution, output: \
  {
    ...
    'gnome-pomodoro': 'The name org.freedesktop.Notifications was not provided by any .service files',
...
test_app_process = \
  ...
  check_app_process_output(
    lambda app, distribution: \
      ...
      echo "$output"
      '''.format(
        'xvfb-run -a -f {} -s "-screen 0 800x600x24" '.format(tuxapp.quote_argument(get_app_xauthority_path(app))) if tuxapp.is_existing_command('xvfb-run') else '',
