...
build_main_page = lambda: \
  join_elements(
    ...
    build_header(),
    build_tag('h2', 'About'),
    build_tag('p', None, '{} is an effort to build an open, community-driven and independent catalog of Linux® applications. We believe that apps should be easily installable, up-to-date and work effortlessly on any Linux® distribution of any version. Being distributed in the binary form it\'s more secure to run them sandboxed, isolated from system and user\'s sensitive files. To seamlessly solve these longstanding problems we\'ve built a tool, {}, that {}'.format(build_tag('strong', '{}.org'.format(get_name())), build_tag('a', None, build_tag('strong', tuxapp.get_name()), href=tuxapp.build_github_url()), tuxapp.get_description()[0].lower() + tuxapp.get_description()[1:])),
    build_tag('h2', 'Features'),
    build_columns(
      build_feature('Works on any distribution', 'Apps from this catalog can be installed and run on almost any Linux® distribution. {} and installed apps don\'t rely on any distribution specific features.'.format(build_code(tuxapp.get_name()))),
      build_feature('Latest app versions', '{} parses the version information online from the app homepages and installs the latest up-to-date versions.'.format(build_code(tuxapp.get_name()))),
      build_feature('Independent from system libraries', 'App authors don\'t include some libraries they assume are present on your system but they may not. {} just detects and downloads all required libraries and tells apps to use them.'.format(build_code(tuxapp.get_name()))),
      ...
      build_feature('No root permissions needed', '{} operates with user permissions and doesn\'t ask for {}. Apps are installed in the home directory at {}.'.format(build_code(tuxapp.get_name()), build_code('sudo'), build_code('~/.tuxapp'))),
      build_feature('Leaves the system intact', '{} doesn\'t clutter the system, and files of installed apps aren\'t scattered throughout the file system.'.format(build_code(tuxapp.get_name()))),
      build_feature('One command install', '{} is just a single {} script doing all the job. E.g., to install {} run in the terminal: {}'.format(build_code(tuxapp.get_name()), build_code('Python'), build_code('firefox'), build_command(build_app_install_command('firefox')))),
