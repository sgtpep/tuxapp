...
class OrderChecker(pylint.checkers.BaseChecker):
  ...
  msgs = {
    ...
    'W0002': ("Class %s is not defined in sorted order", 'unordered-class', ''),
    'W0003': ("Function %s is not defined in sorted order", 'unordered-function', ''),
    'W0004': ("Lambda %s is not defined in sorted order", 'unordered-lambda', ''),
  }
  ...
  def visit_classdef(self, node):
    if not node.col_offset:
      if self.previous_class and self.previous_class > node.name:
        self.add_message('unordered-class', args=node.name, node=node)
